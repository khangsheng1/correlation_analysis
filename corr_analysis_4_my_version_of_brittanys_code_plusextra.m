...
%  11/16/18 
%  The difference between this and the other scripts for finding the
%  correlation is that it normaizes each unit to itself, not the whole
%  population or trial period.
...
%     end
    ...
    for m = 1:unit_number
        max_binned_cue_NR = (max(max(binned_cue_NR(:,:,m))));
        max_binned_cue_R =  (max(max(binned_cue_R(:,:,m))));
        max_binned_rew_NR = (max(max(binned_rew_NR(:,:,m))));
        max_binned_rew_R =  (max(max(binned_rew_R(:,:,m))));
        min_binned_cue_NR = (min(min(binned_cue_NR(:,:,m))));
        min_binned_cue_R =  (min(min(binned_cue_R(:,:,m))));
        min_binned_rew_NR = (min(min(binned_rew_NR(:,:,m))));
        min_binned_rew_R =  (min(min(binned_rew_R(:,:,m))));
        
        mmn_cue_NR_whole(:,:,m) = (binned_cue_NR(:,:,m) - ...
            min_binned_cue_NR)/(max_binned_cue_NR - min_binned_cue_NR);
        mmn_cue_R_whole(:,:,m) = (binned_cue_R(:,:,m) - ...
            min_binned_cue_R)/(max_binned_cue_R - min_binned_cue_R);
        mmn_rew_NR_whole(:,:,m) = (binned_rew_NR(:,:,m) - ...
            min_binned_rew_NR)/(max_binned_rew_NR - min_binned_rew_NR);
        mmn_rew_R_whole(:,:,m) = (binned_rew_R(:,:,m) - ...
            min_binned_rew_R)/(max_binned_rew_R - min_binned_rew_R);
    end
